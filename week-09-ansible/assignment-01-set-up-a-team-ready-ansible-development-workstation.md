...
![alt text](<Screenshot 2026-09-13 041344.png>)
---
...
All Python dependencies are saved in requirements.txt with pinned versions. This means any team member can clone the repository, create a fresh virtual environment, and run pip install -r requirements.txt to get the exact same tools and versions. Nobody has to guess what to install or worry about version mismatches.
---

**2. What is one pitfall you avoided while completing the setup?**


I avoided installing Ansible globally using sudo pip install. A global installation would have mixed project dependencies with system packages, made it impossible to use different Ansible versions across projects, and caused conflicts that are difficult to debug. Using a virtual environment keeps everything scoped to this project only.
---

**3. Why should Ansible be installed inside a Python virtual environment?**


Ansible is a Python-based tool, and installing it inside a virtual environment means its dependencies are isolated from the rest of the system. This prevents conflicts with other Python projects on the same machine, makes the setup reproducible across different workstations, and allows different projects to use different versions of Ansible without interfering with each other.
---

**4. Why must SSH private keys and `.venv/` remain outside version control?**


SSH private keys must never be committed to Git because anyone with access to the repository would be able to use that key to authenticate to your servers — exposing your entire infrastructure. .venv/ should stay out of version control because it contains thousands of generated files tied to a specific machine and Python version. It is not portable, bloats the repository, and is unnecessary since anyone can recreate it from requirements.txt.
---
